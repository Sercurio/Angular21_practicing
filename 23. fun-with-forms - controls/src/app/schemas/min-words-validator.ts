import {aggregateMetadata, customError, LogicFn, maxMetadataKey, metadata, MIN, SchemaPath, validate} from '@angular/forms/signals';

export const MIN_WORDS = maxMetadataKey();

export function minWords(
    path: SchemaPath<string>, minValue: number|LogicFn<string, number>) {
  aggregateMetadata(
      path, MIN_WORDS,
      ctx => typeof minValue === 'number' ? minValue : minValue(ctx));

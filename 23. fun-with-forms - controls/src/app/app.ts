...
import {apply, applyEach, customError, disabled, email, Field, form, max, min, minLength, required, submit, validate, validateTree,} from '@angular/forms/signals';
...
@Component({
  selector: 'app-root',
  imports:
      [CommonModule, Field, FieldStyleDirective, FieldWrapper, ReviewItemForm],
...
export class App {
  ...
  readonly reviewForm = form(this.model, (path) => {
    ...
    minWords(
        path.description,
        (ctx) => ctx.valueOf(path.role) === 'author' ? 10 : 5);
